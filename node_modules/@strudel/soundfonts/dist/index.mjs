    ...
    registerSound(
      ...
      async (l, f, a) => {
        const [r, n, i, G] = getADSRValues([
          ...
          f.sustain,
          f.release
        ]), { duration: o } = f, u = getSoundIndex(f.n, s.length), C = s[u], d = getAudioContext(), t = await getFontBufferSource(C, f, d);
        t.start(l);
        const p = d.createGain(), S = t.connect(p), U = l + o;
        getParamADSR(S.gain, r, n, i, G, 0, 0.3, l, U, "linear");
        let m = U + G + 0.01, F = getVibratoOscillator(t.detune, f, l);
        getPitchEnvelope(t.detune, f, l, U), t.stop(m);
        const R = (M) => {
        };
        return t.onended = () => {
...
Pattern.prototype.soundfont = function(e, s = 0) {
  return this.onTrigger((l, f, a, r, n) => {
    const i = getAudioContext(), G = getPlayableNoteValue(f), o = e.presets[s % e.presets.length], u = n, C = [i, o, noteToMidi(G), u];
    startPresetNote(...C)(u + f.duration);
  });
};
